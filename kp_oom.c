...
void kp_post(struct kprobe *k, struct pt_regs *r, unsigned long flags)
{
    ...
    int retval = regs_return_value(r);

    pr_alert("%s returned %d\n", KALLSYM, retval);
    ...
    //
    // Here we sum up the memcg total_rss to compare it to threshold
    pr_alert("post-handler: CURRENT(%d) flags= %u state= %ld\n", current->pid, tmp_ts->flags, tmp_ts->state);
    if(!tmp_ts) {
	pr_alert("ABORT!\n");
	return;
    }
    ...
    pr_alert("Under %d %% pagecache left in memcg, abort!\n", bail_mark_percent);
    pr_alert("Modifying %s retval to 0\n", KALLSYM);
    regs_set_return_value(r, 0ul);
